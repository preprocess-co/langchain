...
### Preprocess Loader
If you need to convert files and to get the chunks for generating embeddings you can use [PreprocessLoader](/docs/integrations/document_loaders/preprocess).
You shouldn't apply `TextSplitter`s on `Document`s returned by `load()` method, `PreprocessLoader` automatically generates optimal chunks of text based on the original document layout and content semantics.

```python
from langchain.document_loaders import PreprocessLoader

loader = DirectoryLoader("../", loader_cls=PreprocessLeader، loader_kwargs={"api_key": "PREPROCESS_API_KEY"})
docs = loader.load()
```
...
### Text Loader
If you need to load text files, use the `TextLoader`.
```python
