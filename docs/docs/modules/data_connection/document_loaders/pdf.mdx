...
## Using Preprocess
You can use `PreprocessLeader` to load PDF documents and obtain chunks for downstream use in generating embeddings. `PreprocessLoader` automatically creates optimal text chunks based on the original document's layout and content semantics. For this reason, you shouldn't apply `TextSplitter`s to `Document`s returned by the `load()` method.
